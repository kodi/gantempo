...
| Post-M5: React composition and DX refactor | Foundation cleanup before Slice 5 | The public component remains stable while private renderer atoms, DOM adapters, overlays, default surfaces, and the React runtime become cohesive, performance-isolated modules | `[-]` | [Slice 5 complete](plans/2026-08-01-react-composition-and-dx-refactor-plan.md) |
...
**Status:** `[-]` In progress; Slice 5 complete; M5 complete
...
Slices 1 through 5 now lock the black-box structure/render baseline, move pure
...
one per-scene surface model. Ordinary memo boundaries now keep unrelated tasks,
dependencies, and lane rows at zero renders through target focus/selection changes
while changed slot/class callbacks still propagate. One private overlay controller and
portal layer now own transient state, host/theme lifecycle, collision/dismissal,
modal isolation, scroll locking, focus, slot diagnostics, and pending/rejection
behavior; overlay open/close produces zero static content renders. `Gantt.tsx` is
2,287 lines without a DOM, accessibility, style, or public API change. Slice 6 now
owns DOM interaction and measurement adapters.
...
- 2026-08-01: Completed React composition/DX Slice 5. One private overlay controller
  and dedicated portal layer now own host/theme, dismissal/collision, modal isolation,
  scroll locking, focus, diagnostics, and pending/rejection lifecycle. Overlay editor
  open/close records zero static task/lane/dependency content renders, `Gantt.tsx` is
  2,287 lines, focused overlay/SSR coverage passed 10 files / 106 tests, and
  `mise run ci` passed 93 files / 493 tests with a 184-artifact build.

- 2026-08-01: Completed React composition/DX Slice 4. Ordinary memo boundaries and
  stable semantic inputs now isolate task, dependency, and lane-row work without a
  custom comparator; the characterized unrelated Task B/C, lane-row, and dependency
  B–C probes are zero while changed slots/classes still propagate. Focused React
  coverage passed 11 files / 100 tests, the runtime and both scene benchmarks
  completed, and `mise run ci` passed 93 files / 493 tests with a 180-artifact build.

- 2026-08-01: Completed React composition/DX Slice 3. One per-scene surface model now
  supplies indexed task, lane, dependency, column, and accessibility data to private
  semantic chart components; repeated per-lane filtering and dependency searches are
  gone from the render tree. `Gantt.tsx` is 2,705 lines, focused React coverage passed
  11 files / 97 tests, both fixed scene benchmarks completed, and `mise run ci` passed
  93 files / 490 tests with a 180-artifact package build.

- 2026-08-01: Completed React composition/DX Slice 2. Pure presentation and canonical
  editor-command adapters moved out of `Gantt.tsx` with focused tests, and the seven
  default surfaces now live in named private files. `Gantt.tsx` is 3,500 lines; public
  types and root exports are unchanged. Focused coverage passed 6 files / 62 tests,
  and `mise run ci` passed 92 files / 489 tests with a 154-artifact package build.
